    ...
    <LabelsContext.Provider value={toolbarLabels}>
      <>
        <div className={h}>
          ...
          {/* Font size — visible only when text-box tool is active */}
          {activeTool === 'text' && pdfDoc && (
            <div className="flex items-center gap-1 px-1.5 flex-shrink-0">
              <span className={`text-[10px] whitespace-nowrap ${isDark ? 'text-zinc-500' : 'text-gray-400'}`}>Schriftgröße</span>
              <input type="range" min={8} max={24} step={1} value={textFontSize}
                onChange={e => setTextFontSize(Number(e.target.value))}
                className="w-16 accent-clover-500" />
              <span className={`text-[10px] w-4 text-right tabular-nums ${isDark ? 'text-zinc-400' : 'text-gray-500'}`}>{textFontSize}</span>
              <button title="Fett" onClick={() => setTextBold(!textBold)}
                className={`w-6 h-6 rounded text-xs font-bold transition-colors flex-shrink-0
                  ${textBold ? 'bg-clover-600 text-white' : isDark ? 'text-zinc-400 hover:bg-zinc-800' : 'text-gray-500 hover:bg-gray-100'}`}>
                B
              </button>
            </div>
          )}
        ...
        {showNewFieldBar && (
          <div className={`flex items-center gap-3 px-4 py-1.5 text-xs border-b
            ...
            <div className="flex gap-1">
              {[['text', 'Textfeld'], ['checkbox', 'Kontrollkästchen'], ['dropdown', 'Dropdown'], ['listbox', 'Listenfeld'], ['radio', 'Radio-Gruppe'], ['date', 'Datum'], ['signature', 'Unterschrift']].map(([v, l]) => (
