...
async function flattenAnnotations(pdfBytes, annotations, formValues = {}) {
  ...
  const hexRgb = (hex) => {
    ...
    return pRgb(parseInt(c.slice(0,2),16)/255, parseInt(c.slice(2,4),16)/255, parseInt(c.slice(4,6),16)/255)
  }
  ...
  for (const [pgStr, anns] of Object.entries(byPage)) {
    const page = doc.getPage(Number(pgStr) - 1)
    if (!page) continue
    ...
    for (const a of anns) {
      ...
      if (a.rects?.length) {
        ...
        const fSz = 9
        if (a.type === 'note') {
          page.drawRectangle({ x: tx, y: ty - 14, width: 14, height: 14, color: pRgb(1,0.8,0), borderWidth: 0 })
          page.drawText(a.text.slice(0, 60), { x: tx + 16, y: ty - 10, size: fSz, font: f, color: pRgb(0,0,0) })
        } else {
          const lines = a.text.split('\n').slice(0, 5)
          const boxW  = Math.max(...lines.map(l => f.widthOfTextAtSize(l.slice(0,35), fSz)), 40) + 8
          const boxH  = lines.length * (fSz * 1.4) + 6
          page.drawRectangle({ x: tx, y: ty-boxH, width: boxW, height: boxH, color: pRgb(1,1,1), borderColor: pRgb(0.5,0.5,0.5), borderWidth: 0.7, opacity: 0.95 })
          lines.forEach((line, i) => page.drawText(line.slice(0,35), { x: tx+4, y: ty - fSz*1.4*(i+1)+2, size: fSz, font: f, color: pRgb(0,0,0) }))
        }
      }
    }
  }
...
function PDFPage({ pageNum }) {
  ...
  const onMouseDown = (e) => {
    ...
    if (!['draw', 'note', 'text', 'redact', 'eraser'].includes(tool)) return
    e.preventDefault()
  ...
  const isDrawTool     = ['draw', 'note', 'text', 'redact', 'eraser'].includes(activeTool)
  const isTextAnnotTool = ['highlight', 'underline', 'strikethrough'].includes(activeTool)
